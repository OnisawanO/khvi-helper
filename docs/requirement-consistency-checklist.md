...
| Remember me | เลือก persistent หรือ session cookie | ปิด/เปิด browser แล้ว session อยู่ต่อเฉพาะกรณีที่เลือก | ไม่เปลี่ยน schema; ใช้ cookie marker ที่ไม่ใช่ token | `utils/supabase/{client,server,middleware}.ts`, `proxy.ts` | สอดคล้อง |
