...
| `/map` | Resource map/list | Approved Interpreter (ยังไม่บังคับใน mock) | OpenStreetMap tiles + mock open requests (วางแผนเปลี่ยนคำขอเป็น `bookings`, interpreter skills) | Empty state; `403` เมื่อเชื่อม auth จริง | Implemented (mock data, real map) |
