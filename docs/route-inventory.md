...
| `/api/auth/login` | Auth action route | Public | Supabase Auth password login; server-side profile/role check; unchecked `rememberMe` uses a browser-session cookie, checked `rememberMe` uses a fixed absolute cookie deadline of at most 15 days | `400` validation; `401` credentials; `403` locked or unavailable profile | Implemented at `app/api/auth/login/route.ts` |
...
`/login` เป็นหน้า Login โดยตรงที่ใช้ฟอร์ม Supabase Auth เดียวกับ modal หน้าแรก พร้อม checkbox `จดจำฉัน` ที่เริ่มต้นไม่เลือก. เมื่อเลือก cookie จะมี deadline แบบ absolute ไม่เกิน 15 วันนับจาก Login และเมื่อไม่เลือก cookie จะหมดอายุเมื่อปิด browser; หน้าแรกยังเปิด Login modal ได้ ส่วน `/sign-in` redirect ไป `/?signin=true` เพื่อคง compatibility เดิม
