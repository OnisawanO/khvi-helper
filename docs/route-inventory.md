...
- Mission detail watches the current actor's browser geolocation while an active mission page is open and persists live coordinates through the authenticated mission-location flow. Each role sees its own marker update automatically; the other party's exact marker appears only after requester confirmation. The shared map supports an in-page full-screen view, mouse or touch dragging, and pinch-to-zoom on touch screens. Tracking stops when the page closes or the mission is no longer active. Missing locations remain empty instead of using invented coordinates.
