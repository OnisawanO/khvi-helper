...
export default async function MyRequestsPage(props: PageProps<"/user/my-requests">) {
  const { status } = await props.searchParams;
  const activeFilter = resolveStatusFilter(status ?? "completed");
  const requests = await loadRequesterRequests();

  return (
    <WorkspaceShell requiredRole="User" alternatePath="/interpreter/my-assignments#main-content">
