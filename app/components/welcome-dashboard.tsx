...
export function WelcomeDashboard({
  ...
  function requestRow(r: HelpRequest, discovery = false, showRequestId = true) {
    return <li key={r.requestId} className="flex flex-wrap items-center justify-between gap-4 py-5">
      <div className="min-w-0">{showRequestId && <p className="text-xs text-(--khvi-ink)/60">#{r.requestId}</p>}<h3 className={showRequestId ? "mt-1 font-bold" : "font-bold"}>{lang(r)} · {cat(r)}</h3><p className={muted}>{discovery ? r.areaName : r.scheduledAtLabel ?? r.createdAtLabel}</p></div>
