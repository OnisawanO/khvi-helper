...
3. คำขอมีสองรูปแบบที่ต้องแสดงแตกต่างกันบนแผนที่: งาน `Immediate` และงาน `Scheduled` ซึ่งเลือกเวลานัดได้ตั้งแต่วันถัดไปตามปฏิทินโดยไม่จำกัดวันสูงสุด โดยงาน Scheduled แสดงทันทีหลังสร้าง
...
* **BR-07 (Pin Timeout & Auto-Expire):** หมุดเร่งด่วนที่ไม่มีล่ามกดรับภายใน 30 นาทีจะถูกเปลี่ยนสถานะเป็น `Expired` อัตโนมัติ ส่วนงานนัดหมายต้องเลือกเวลาตั้งแต่วันถัดไปตามปฏิทินโดยไม่จำกัดวันสูงสุด และแสดงรูปแบบแตกต่างจากหมุดเร่งด่วน
...
* **BR-10 (One Active Workspace Task):** หนึ่งบัญชีมีคำขอหรืองานที่ยังดำเนินการอยู่ได้ไม่เกินหนึ่งรายการข้ามโหมด User/Interpreter; `Open` ที่ยังไม่หมดอายุ, `Claimed` และ `InProgress` จะบล็อกการสร้างคำขอใหม่และการ Claim งานใหม่ แต่ยังสลับโหมดเพื่อดูข้อมูลได้
* **BR-11 (Requester Edit Window):** เจ้าของคำขอแก้ไขภาษา หมวดหมู่ รายละเอียด และจุดนัดพบได้ในสถานะ `Open` หรือ `Claimed` และแก้ไขไม่ได้ตั้งแต่สถานะ `InProgress` เป็นต้นไป
...
erDiagram
    ...
    BOOKING {
        ...
        TIMESTAMPTZ scheduled_at "เวลานัดหมาย (ตั้งแต่วันถัดไป ไม่จำกัดวันสูงสุด)"
...
│   ├── user/                         <── Welcome, request-help และ my-requests ของ User
│   ├── interpreter/                  <── Welcome, find-requests และ my-assignments ของ Interpreter
│   ├── welcome/                      <── compatibility redirect ตาม role
│   ├── login/, register/             <── [คนที่ 1] หน้า Auth แบบ direct route
│   ├── request-help/, my-requests/   <── compatibility redirect ไป `user/`
│   ├── find-requests/, my-assignments/ <── compatibility redirect ไป `interpreter/`
...
│   ├── mission/                      <── [คนที่ 5] ContactCard, ExecutionControls, CompletionConfirm สำหรับ `/user/my-requests/[requestId]`
...
| **คนที่ 2** | **SOS Pin Creation & Requester Hub**<br>*(ระบบปักหมุดและจัดการคำขอ)* | • `app/user/page.tsx`<br>• `app/user/request-help/page.tsx`<br>• `app/user/my-requests/page.tsx`<br>• `app/user/my-requests/[requestId]/page.tsx`<br>• `components/pin-request/PinForm.tsx` | • ตาราง `bookings` (Insert `status = 'Open'`)<br>• Geolocation API (ดึง GPS)<br>• `actions/pin-actions.ts` |
| **คนที่ 3** | **Interactive SOS Map & Visual Discovery**<br>*(ระบบแผนที่และตัวกรองตามความสามารถ)* | • `app/interpreter/page.tsx`<br>• `app/interpreter/find-requests/page.tsx`<br>• `components/map/LeafletMap.tsx`<br>• `components/map/CustomMarkers.tsx`<br>• `components/map/MapFilterBar.tsx`<br>• `components/map/PinSummaryModal.tsx` | • Query `bookings` (`status = 'Open'`) โดย match `language_id`, `category_id`<br>• แสดงตำแหน่งคร่าว ๆ ก่อน claim และแยกประเภทงานเร่งด่วน/นัดหมาย |
| **คนที่ 4** | **Volunteer Portal & Matching Engine**<br>*(ระบบรับสมัครล่ามและรับงาน)* | • `app/interpreter/my-assignments/page.tsx`<br>• `app/volunteer/apply/page.tsx`<br>• `app/volunteer/dashboard/page.tsx`<br>• `app/volunteer/status/page.tsx`<br>• `components/volunteer/ClaimButton.tsx` | • ตาราง `interpreter_profiles`, `languages`, `categories`, `interpreter_languages`, `interpreter_categories`<br>• **Postgres RPC `claim_booking`** (Atomic Concurrency Lock)<br>• `actions/volunteer-actions.ts` |
